import csv

def validate_asrama_tempatTinggal(r81_file, asrama_file, mahasiswa_file):
    # Membaca data dari csv asrama_attribute.csv
    asrama_data = {}
    with open(asrama_file, 'r', newline='', encoding='utf-8') as csvfile:
        csvreader = csv.reader(csvfile, delimiter=',')
        next(csvreader, None)  # Lewati header
        for row in csvreader:
            nama_asrama = row[0].strip()  # Kolom namaAsrama
            asrama_data[nama_asrama] = True  # Gunakan nilai boolean sederhana karena tidak perlu data lain dari asrama

    # Membaca data dari csv mahasiswa_attribute.csv
    mahasiswa_data = {}
    with open(mahasiswa_file, 'r', newline='', encoding='utf-8') as csvfile:
        csvreader = csv.reader(csvfile, delimiter=',')
        next(csvreader, None)  # Lewati header
        for row in csvreader:
            nomor_induk_mahasiswa = row[4].strip()  # Kolom nomorIndukMahasiswa
            mahasiswa_data[nomor_induk_mahasiswa] = True  # Gunakan nilai boolean sederhana karena tidak perlu data lain dari mahasiswa

    # Validasi data di R81_Asrama_tempatTinggal.csv
    errors = []
    with open(r81_file, 'r', newline='', encoding='utf-8') as csvfile:
        csvreader = csv.reader(csvfile, delimiter=',')
        next(csvreader, None)  # Lewati header
        for line_num, row in enumerate(csvreader, start=2):
            nama_asrama = row[0].strip()  # Kolom namaAsrama
            nomor_induk_mahasiswa = row[1].strip()  # Kolom nomorIndukMahasiswa

            # Validasi namaAsrama
            if nama_asrama not in asrama_data:
                errors.append(f"Data salah pada baris {line_num}: namaAsrama {nama_asrama} tidak ditemukan di asrama_attribute.csv")

            # Validasi nomorIndukMahasiswa
            if nomor_induk_mahasiswa not in mahasiswa_data:
                errors.append(f"Data salah pada baris {line_num}: nomorIndukMahasiswa {nomor_induk_mahasiswa} tidak ditemukan di mahasiswa_attribute.csv")

    # Cetak hasil validasi
    if errors:
        for error in errors:
            print(error)
    else:
        print(f"Validasi sukses! Semua data pada {r81_file} sesuai.")

# Contoh pemanggilan fungsi validate_asrama_tempatTinggal
r81_file = r'C:\Users\RAJA\project\fukg\university_dormitory\R81_Asrama_tempatTinggal.csv'
asrama_file = r'C:\Users\RAJA\project\fukg\university_dormitory\asrama_attribute.csv'
mahasiswa_file = r'C:\Users\RAJA\project\fukg\university_student\mahasiswa_attribute.csv'

validate_asrama_tempatTinggal(r81_file, asrama_file, mahasiswa_file)
