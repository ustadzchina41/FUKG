...
def validate_jurnal_csv(csv_file):
    ...
    with open(csv_file, 'r', newline='', encoding='utf-8') as csvfile:
        ...
        for line_num, row in enumerate(csvreader, start=2):
            ...
            editor = row[1].strip()
            if ',' in editor:
                errors.append(f"Data salah pada baris {line_num}, informasi error: Editor tidak boleh mengandung koma")
            if not re.match(r'^[A-Z][a-z]*(\s[A-Z][a-z]*)*$', editor):
                errors.append(f"Data salah pada baris {line_num}, informasi error: Editor harus Kapital setiap kata")
...
csv_path = r'C:\Users\RAJA\project\fukg\scientific_journal\jurnal_attribute.csv'
